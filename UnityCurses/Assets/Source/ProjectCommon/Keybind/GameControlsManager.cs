...
namespace Assets.Source.ProjectCommon.Keybind
{
    ...
    {
        private const string PlayerPrefsKeybinds = "Keybinds";
        ...
        private bool InitInternal()
        {
            //create items
            {
                ...
                // Check if the keybinds key exists in the player preferences cache.
                if (PlayerPrefs.HasKey(PlayerPrefsKeybinds))
                {
                    LoadCustomConfig();
                }
                else
        ...
        {
            var block = new KeybindData();
            foreach (var item in Items)
            {
                // Keyboard Setting
                var currentKeyBlock = block.AddChild(item.ControlKey.ToString());
                if (item.BindedKeyboardMouseValues.Count <= 0)
                    continue;

                foreach (var keyboardvalue in item.BindedKeyboardMouseValues)
                {
                    currentKeyBlock.SetAttribute("type", keyboardvalue.Type.ToString());
                    switch (keyboardvalue.Type)
                    {
                        case SystemKeyboardMouseValue.Types.Key:
                            currentKeyBlock.SetAttribute("key", keyboardvalue.Key.ToString());
                            break;
                        case SystemKeyboardMouseValue.Types.MouseScrollDirection:
                            currentKeyBlock.SetAttribute("scroll", keyboardvalue.ScrollDirection.ToString());
                            break;
                        default:
            ...
            // Serialize the game keybind configuration, set it into Unity player preferences.
            var output = Serializer.SerializeObject(block);
            PlayerPrefs.SetString(PlayerPrefsKeybinds, output);

            // Saves the player preferences data from unity to players disk.
            PlayerPrefs.Save();
        }

        public void LoadCustomConfig()
        {
            // Only print out error if keybinds key doesn't exist.
            var keybindBlob = PlayerPrefs.GetString(PlayerPrefsKeybinds, string.Empty);
            if (string.IsNullOrEmpty(keybindBlob))
            {
                Debug.Log("GameControlsManager: No keybinds configuration found, resetting to defaults...");
                ResetKeyMouseSettings();
                return;
            }

            // Parse the data we loaded back into keybinding object.
            var controlBloc = Serializer.DeserializeObject(keybindBlob) as KeybindData;
            if (controlBloc == null)
            {
            ...
            foreach (var item in Items)
            {
                ...
                if (currentKeyBlock == null || currentKeyBlock.Binds.Count <= 0)
                    continue;

                // keyboard Setting
                var value = new SystemKeyboardMouseValue();
                var type = currentKeyBlock.GetAttribute("type");
                if (!string.IsNullOrEmpty(type))
                    value.Type = (SystemKeyboardMouseValue.Types) Enum.Parse(typeof(SystemKeyboardMouseValue.Types), type);

                var key = currentKeyBlock.GetAttribute("key");
                if (!string.IsNullOrEmpty(key))
                    value.Key = (KeyCode) Enum.Parse(typeof(KeyCode), key);

                var scroll = currentKeyBlock.GetAttribute("scroll");
        ...
        public class SystemKeyboardMouseValue : SystemControlValue
        {
            ...
            public MouseScroll ScrollDirection { get; set; }
