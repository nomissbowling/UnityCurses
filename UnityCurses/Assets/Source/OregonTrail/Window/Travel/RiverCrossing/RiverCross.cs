...
using Assets.Source.OregonTrail.Window.Travel.Rest;
using Assets.Source.OregonTrail.Window.Travel.RiverCrossing.Ferry;
using Assets.Source.OregonTrail.Window.Travel.RiverCrossing.Help;
using Assets.Source.OregonTrail.Window.Travel.RiverCrossing.Indian;

namespace Assets.Source.OregonTrail.Window.Travel.RiverCrossing
{
    /// <summary>
    ...
    public sealed class RiverCross : Form<TravelInfo>
    {
        ...
        public override void OnFormPostCreate()
        {
            ...
            _riverInfo.AppendLine(string.Format("{0}", riverLocation.Name));
            _riverInfo.AppendLine(string.Format("{0}", game.Time.Date));
            _riverInfo.AppendLine("--------------------------------");
            _riverInfo.AppendLine(
                string.Format("Weather: {0}", riverLocation.Weather.ToDescriptionAttribute()));
            _riverInfo.AppendLine(string.Format("River width: {0:N0} feet", UserData.River.RiverWidth));
            _riverInfo.AppendLine(string.Format("River depth: {0:N0} feet", UserData.River.RiverDepth));
            _riverInfo.AppendLine("--------------------------------");
            _riverInfo.AppendLine(string.Format("You may:{0}", Environment.NewLine));
        ...
        private void AddRiverChoice(RiverCrossChoice riverChoice)
        {
            ...
            switch (riverChoice)
            {
                ...
                default:
                    throw new ArgumentOutOfRangeException("riverChoice",
