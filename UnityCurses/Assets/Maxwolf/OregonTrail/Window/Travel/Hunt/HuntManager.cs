...
namespace Assets.Maxwolf.OregonTrail.Window.Travel.Hunt
{
    ...
    public sealed class HuntManager : ITick
    {
        ...
        /// <summary>
        ///     Initializes a new instance of the <see cref="T:Assets.Maxwolf.OregonTrail.Window.Travel.Hunt.HuntManager" /> class.
        /// </summary>
        public HuntManager()
        {
            ...
            // Init animals for the player to shoot with their bullets.
            GeneratePrey();
        }
        ...
        public string HuntInfo
        {
            get
            {
                ...
                huntStatus.AppendLine(string.Format("{0}--------------------------------", Environment.NewLine));
                ...
                huntStatus.AppendLine(game.Trail.CurrentLocation.Status != LocationStatus.Departed
                    ? string.Format("Hunting outside {0}", game.Trail.CurrentLocation.Name)
                    : string.Format("Hunting near {0}", game.Trail.NextLocation.Name));

                // Represent seconds remaining as daylight left percentage.
                var daylightPercentage = _secondsRemaining / (decimal) HUNTINGTIME;
                huntStatus.AppendLine(string.Format("Daylight Remaining: {0:N0}%", daylightPercentage * 100));

                // Current weather on the planes.
                huntStatus.AppendLine(string.Format("Weather: {0}",
                    game.Trail.CurrentLocation.Weather.ToDescriptionAttribute()));
                huntStatus.AppendLine("--------------------------------");

                // Show the player their current shooting word and target they are aiming at.
                huntStatus.AppendLine(
                    string.Format("{0}Shooting Word: {1}", Environment.NewLine,
                        ShootingWord.ToString().ToUpperInvariant()));

                // Only show the target to shoot at if there is one.
                huntStatus.AppendLine(_target != null
                    ? string.Format("Target: {0}{1}", _target.Animal.Name.ToUpperInvariant(), Environment.NewLine)
                    : "Target: NONE");
                ...
                if (_target != null)
                {
                    ...
                    huntStatus.AppendLine(string.Format("Awareness: {0:N0}%", targetPercentage * 100));
                }

                // Prompt the player with information about what to do.
                if (ShootingWord != HuntWord.None)
                {
                    huntStatus.AppendLine(string.Format("Type the word '{0}' to",
                        ShootingWord.ToString().ToLowerInvariant()));
                ...
                else
                {
                    ...
                    huntStatus.AppendLine(
                        string.Format("{0}You sense {1:N0} {2}", Environment.NewLine, _sortedPrey.Count,
                            animalText));
        ...
        private void GeneratePrey()
        {
            ...
            // Init the number of prey required by the dice roll.
        ...
        public bool TryShoot()
        {
            ...
            if (100 * EngineApp.Random.Next() <
                ((int) game.Vehicle.PassengerLeader.Profession - 13) * _target.TargetTime)
            {
