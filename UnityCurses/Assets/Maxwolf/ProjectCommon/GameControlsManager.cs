...
namespace Assets.Maxwolf.ProjectCommon
{
    ...
    {
        private const string KeybindPlayerPref = "Keybinds";
        ...
        private bool InitInternal()
        {
            //create items
            {
                ...
                if (PlayerPrefs.HasKey(KeybindPlayerPref))
                {
                    LoadCustomConfig();
                }
                else
        ...
        public void SaveCustomConfig()
        {
            ...
            var jsonBlock = JSONExtensions.Serialize(block, false);
            PlayerPrefs.SetString(KeybindPlayerPref, jsonBlock);
            PlayerPrefs.Save();
        }
        ...
        public void LoadCustomConfig()
        {
            ...
            try
            {
                customblock = JSONExtensions.Deserialize<TextBlock>(PlayerPrefs.GetString(KeybindPlayerPref), false);
            }
            catch (Exception)
            {
                Debug.LogWarning("Error deserializing keybind data, resetting it to defaults...");

                // Removes player preferences incase something is very wrong with them.
                PlayerPrefs.DeleteKey(KeybindPlayerPref);
                PlayerPrefs.Save();
            ...
            {
                ResetKeyMouseSettings();
                Debug.LogWarning("No custom keyboard and mouse binds found, skipping file load...");
                return;
            }
