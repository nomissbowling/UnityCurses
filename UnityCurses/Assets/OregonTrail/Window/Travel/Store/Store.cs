...
using Assets.OregonTrail.Window.Travel.Store.Help;

namespace Assets.OregonTrail.Window.Travel.Store
{
    /// <summary>
    ...
    public sealed class Store : Form<TravelInfo>
    {
        ...
        private void UpdateStore()
        {
            ...
            {
                _storePrompt.AppendLine(string.Format("{0} General Store",
                    OregonTrailApp.Instance.Trail.CurrentLocation.Name));
                _storePrompt.AppendLine(string.Format("{0}", OregonTrailApp.Instance.Time.Date));
                _storePrompt.AppendLine("--------------------------------");
                    ...
                    var storeTag = storeItem.ToDescriptionAttribute()
                        .Replace("@AMT@",
                            UserData.Store.Transactions[storeItem].ToString(
                                OregonTrailApp.Instance.Trail.IsFirstLocation &&
                                OregonTrailApp.Instance.Trail.CurrentLocation.Status == LocationStatus.Unreached));

                    // Last line should not print new line.
                    if (index == storeAssets.Count - 5)
                    {
                        _storePrompt.AppendLine(string.Format("  {0}. {1}", (int) storeItem, storeTag));
                        _storePrompt.AppendLine(string.Format("  {0}. Leave store", storeAssets.Count - 3));
                    }
                    else
                    {
                        _storePrompt.AppendLine(string.Format("  {0}. {1}", (int) storeItem, storeTag));
                ...
                var amountPlayerHas = OregonTrailApp.Instance.Vehicle.Balance - totalBill;

                // If at first location we show the total cost of the bill so far the player has racked up.
                _storePrompt.Append(OregonTrailApp.Instance.Trail.IsFirstLocation &&
                                    OregonTrailApp.Instance.Trail.CurrentLocation.Status == LocationStatus.Unreached
                    ? string.Format("Total bill:            {0:C2}", totalBill) +
                      string.Format("{0}Amount you have:       {1:C2}", Environment.NewLine, amountPlayerHas)
                    : string.Format("You have {0:C2} to spend.", OregonTrailApp.Instance.Vehicle.Balance));
            }
        }
...
#pragma warning restore IDE0018 // Inline variable declaration
            EnumExtensions.TryParse(input, out selectedItem);
        ...
        private void LeaveStore()
        {
            ...
            if (OregonTrailApp.Instance.Trail.CurrentLocation != null && OregonTrailApp.Instance.Trail.IsFirstLocation &&
                OregonTrailApp.Instance.Trail.CurrentLocation.Status == LocationStatus.Unreached)
            {
                ...
                OregonTrailApp.Instance.Trail.ArriveAtNextLocation();
